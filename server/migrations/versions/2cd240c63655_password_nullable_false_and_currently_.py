...
def upgrade():
    ...
    sa.Column('relation_cat', sa.Integer(), nullable=False),
    sa.Column('close_friend', sa.Boolean(), nullable=True),
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['receiver_id'], ['users.id'], name=op.f('fk_interactions_receiver_id_users')),
    sa.ForeignKeyConstraint(['sender_id'], ['users.id'], name=op.f('fk_interactions_sender_id_users')),
    sa.PrimaryKeyConstraint('id')
    )
